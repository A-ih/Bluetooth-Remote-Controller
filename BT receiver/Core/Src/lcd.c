...
// here are never used
void LCD_DisplayData(uint16_t usC, uint16_t usP, uint16_t usColor)
{
	uint8_t i, j;
	uint8_t temp;

	for(i = 0; i < 16; i++) // For each row
	{
		for(j = 0; j < 2; j++) // Each byte contains 8 pixels
		{
			temp = data[i * 2 + j]; // Get current byte
			
			// Process each bit in the byte
			for(uint8_t k = 0; k < 8; k++)
			{
				if(temp & (0x80 >> k)) // If bit is 1, draw dot
				{
					LCD_DrawDot(usC + j * 8 + k, usP + i, usColor);
				}
			}
		}
	}
